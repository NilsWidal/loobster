---
name: run
description: Run the full RePPITS workflow (Research -> Propose -> Plan -> Implement -> Test -> Secure) with risk-tiered approval gates. Use to build or change a feature end-to-end. Not for tiny one-off edits.
---

<!-- GENERATED from commands/run.md by bin/build-codex-skills.py — do not edit here. -->
...
Follow the instructions in `.agents/skills/research-codebase/SKILL.md`.
...
Follow the instructions in `.agents/skills/make-proposals/SKILL.md`, using the research from Phase 1.
...
Follow the instructions in `.agents/skills/make-plan/SKILL.md`, using the chosen proposal from Phase 2.
...
After all sub-issues are implemented, follow `.agents/skills/review-code/SKILL.md` to review all changes.
...
Follow `.agents/skills/secure/SKILL.md` to run security checks against all changes, for the **enabled frameworks** (any of HIPAA, HITRUST, ISO 27001, SOC 2 — configured via `.claude/loobster-frameworks.json`; see `compliance/frameworks.md`).
...
- If there are FAIL items, run the **bounded autonomous convergence loop** (no gate between iterations — this is the one place the workflow self-drives):
  ...
  - **Iteration cap: 3 — then a resolver, then the human.** When the cap is reached, do **not** escalate yet: spawn ONE fresh **resolver subagent** with clean context (use the `resolver` model preference from `.claude/loobster.json`, else a model different from the implementer) and hand it the remaining FAILs, a short history of what each iteration tried, and the diff — instructing it to take a **different approach**, with 2 more bounded iterations and the same independent verification. Escalate to the human only if the resolver also fails to converge (total bound: 5 iterations). **Sensitive-tier FAILs that survive the resolver always escalate — no exceptions.** **Never** silently commit or push past unresolved FAILs.
...
- **Final commit/push approval — this stop happens in _both_ modes** (autonomous mode does not bypass it; you never auto-push). **One exception:** when this run is the act step of a goal-loop whose intake granted **`deliveryMode: pr-lane`**, the approval is satisfied *asynchronously* — commit to a feature branch, push it, open a PR (**never the default branch**), and return control to the loop; the PR review is the human approval. Standalone runs and `interactive` mode ask as follows. When clean (or warnings acknowledged), ask: "Security check passed. Ready to commit and push?"
...
- **Never self-verify.** Every verification, review, or judgment step — Test (Phase 5), Secure (Phase 6), frontend verify, and the loop's verify — MUST run in a **separate verifier subagent** (`Agent`) that did **not** produce the work under review. The agent that wrote the code never reviews its own output; spin up an independent judge that gets the diff + criteria and returns only its verdict/findings. This holds in every mode, including autonomous (independence is exactly what keeps an unattended loop honest). If subagents are unavailable, say so and treat the verification as provisional pending an independent reviewer.
- Apply the token-discipline conventions in `.agents/skills/token-discipline/SKILL.md` throughout: delegate heavy reads to subagents and forward only conclusions, pass artifact summaries between phases (re-reading files on demand), and keep stable context stable.
- **Preferred subagents (configurable).** Before spawning any `Agent`, read `.claude/loobster.json` if it exists (example: `reference/loobster.example.json`). Its `subagents` map sets a preferred model per delegation role — `research` (heavy reads: research-codebase, explore), `act` (implement/build), `verify` (the independent Test/Secure/review judge) — passed as the Agent call's `model` override. No file or no entry → inherit the session model (never guess a model id). Two intents this serves: **route the expensive thinking** (e.g. a strong model like Opus for `act`, a cheap one for mechanical `research` reads), and **strengthen never-self-verify** — when `verify` is a *different* model than `act`, the judge doesn't share the builder's blind spots. If the runtime's Agent tool has no model override, say so once and proceed with defaults.
