...
<!-- GENERATED from commands/make-plan.md by bin/build-codex-skills.py — do not edit here. -->
...
- Use `templates/design-doc-template.md` as a reference for what sections to cover.
...
4. Regardless of whether Linear is available, also create Claude Code Tasks using `TaskCreate` for each sub-task. This provides persistent task tracking that survives across sessions.
   - **When invoked under a goal-loop** (`/loop`): tag each Task's `metadata` with the active `goalId` and a RICE score per `.agents/skills/backlog-scoring/SKILL.md`, so the sub-tasks join the prioritized backlog.
