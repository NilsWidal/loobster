...
- **For unattended / persistent looping** (close the laptop, overnight, CI): wrap it with a driver that re-invokes it — the `/loop` skill running `/loobster:loop <goal>`, a scheduled cloud agent, or an Agent SDK harness. The command resumes from the backlog each time. The plugin defines the behavior; the driver supplies the turns.
- **Crash resilience (a dead turn must not kill the loop).** A turn can die mid-response — API connection drop, crash, or a manual stop — and the loop can't re-invoke *itself* from a dead turn. Two mechanisms cover this: (1) the **durable driver** above — its next tick re-invokes `/loobster:loop <goal>`, which **auto-resumes from the checkpoint** (see *Self-healing* below); this is the reliable path for hard crashes. (2) For interactive runs, the bundled **Stop hook** (`hooks/hooks.json` → `bin/loop-rearm.py`) is a best-effort safety net: while a loop is marked `status: active`, it refuses a premature stop and tells the model to resume. Disable with `LOOBSTER_LOOP_REARM=0`.
...
1. **Reload** the goal task (`metadata.kind=goal`) + `plans/loop/<slug>.md` (cycle, learnings). If the goal task is gone but the marker exists, rebuild from `TaskList` for the `goalId`.
2. **Reclaim stale in-progress.** Any task `in_progress` whose `metadata.heartbeatAt` is missing or older than ~2 cycles was **interrupted, not running** → it becomes the next item. **Check what already landed first** (git status/diff, committed sub-issues, sub-task states) and **continue** it — never redo completed work, never double-commit. (This is the M19-style "stranded in_progress after a connection drop" case.)
3. Then enter the loop. (`/resume` routes a goal-loop straight here.)
...
- **Exit** only when one of these holds: (a) the model judges the goal **met**, (b) the backlog is empty with no new work justified, (c) `maxCycles` reached, or (d) `--budget` exhausted. On exit, **set `status: done` in `plans/loop/<slug>.md`** (this releases the Stop-hook re-arm), then summarize: goal status, what shipped (commits/PRs), remaining backlog, key learnings.
- **Escalate** (stop + summarize + set `status: done`, hand back) only on: a stuck Investigate&Act loop (its cap), a sensitive change that fails Secure, or a per-cycle token spike. Never auto-push past unresolved work.
