...
5. Then enter the loop, refreshing the lease each cycle while you hold it — `bin/loop-lease.py refresh plans/loop/<slug>.md <runner-id>` — and mirror `runnerHeartbeatAt` in the marker. (`/resume` routes a goal-loop straight here, and must re-acquire the lease too.)
