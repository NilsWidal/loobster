---
name: loop
description: Run a continuous goal-loop: work a prioritized RICE-scored backlog toward a standing goal, cycle after cycle, consuming/emitting signals. Use for ongoing autonomous improvement toward a goal; not for a single discrete task (use run).
---

<!-- GENERATED from commands/loop.md by bin/build-codex-skills.py — do not edit here. -->
...
3. **Seed the backlog:** `TaskCreate` one task per known work item, each with `metadata.goalId=<slug>` and a RICE score per `.agents/skills/backlog-scoring/SKILL.md`. For large scoped items, run `/make-plan` to decompose — its sub-tasks join the backlog.
4. **Arm self-re-entry and SHOW the schedule (this is what makes the single command self-driving — and visible).** Like the `/loop` skill confirms what it scheduled, this loop **prints its own re-entry settings on kickoff** so you can see exactly what's running, no separate script:
   - If `CronCreate`/`ScheduleWakeup` are callable, arm them — `ScheduleWakeup(prompt: "/loobster:loop <goal>", delaySeconds: 1200)` for an in-session dead-turn fallback (idle ticks past the 5-min cache window are wasted, so lean 1200–1800s), plus `CronCreate` the same prompt for closed-session durability. **The single-runner lease (see *Self-healing*) makes this safe even on a shared worktree:** a fired wakeup/cron **backs off** if a live runner holds the lease and only takes over when the lease goes stale — so you can arm cron instead of hand-skipping it to dodge contention. Record the id in the marker frontmatter (`reentry: <id>`), then **print the schedule**: the **cron expression**, the **human-readable cadence**, the **job/wakeup id**, that recurring crons **auto-expire after 7 days**, and **how to cancel** (`CronDelete <id>` or set `status: done`). Lead the line with **`self-driving: ARMED`**.
...
2. **Acquire the single-runner lease (concurrency safety — only ONE loop instance runs at a time per worktree).** Read `runner` + `runnerHeartbeatAt` from the marker. If `runnerHeartbeatAt` is **fresh** (within ~2 cycles / a few minutes) and `runner` isn't you, **another instance is already looping** — this is exactly the case of a `ScheduleWakeup`/cron firing while an in-session run is live, or a parallel headless run on the same shared worktree. Do **not** start a concurrent cycle: leave the fallback armed and **exit quietly**. If the lease is **stale or empty**, claim it — write `runner: <a fresh id>` + `runnerHeartbeatAt: now` — and proceed. This mutex is what makes self-re-entry collision-proof (the reason cron can be armed rather than hand-skipped).
...
5. Then enter the loop, refreshing `runnerHeartbeatAt` each cycle while you hold the lease. (`/resume` routes a goal-loop straight here.)

### The loop — repeat until an exit condition
1. **Trigger (next item):** first **consume relevant signals** from the shared hub (see `.agents/skills/signals/SKILL.md`) — read `signals/*.md` with `status: new|ack` and a `relevance` tag this goal cares about; a high-confidence signal may outrank the backlog (or spawn a new scored task). Then `TaskList`, filter to open, unblocked tasks for this `goalId`, pick the highest `metadata.score` (ties → lowest effort, then lowest id). If none remain → go to step 4 to decide done-vs-new-work.
