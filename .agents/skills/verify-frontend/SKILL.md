...
4. **Attach to the PR — GitHub-native, no other accounts.** Two ways:
   - **CI artifacts (default, zero repo clutter):** copy `templates/playwright-verify.yml` into your repo's `.github/workflows/`; it runs this on the PR and `upload-artifact`s `.loobster/screens/` — downloadable from the PR's **Checks** tab. It also posts a sticky PR comment with the result table + artifact link.
