---
name: signals
description: Emit or consume cross-loop signals (observations / frictions / opportunities) in the shared signals/ store. Use to record a finding for other loops or teammates, or to read relevant ones before deciding. PHI must never go in a signal.
---

<!-- GENERATED from commands/signals.md by bin/build-codex-skills.py — do not edit here. -->
...
2. On acting on a signal: set `status: acted`, and optionally `TaskCreate` a RICE-scored backlog task (see `.agents/skills/backlog-scoring/SKILL.md`) — signals are **upstream** of the backlog.
...
- The PHI-exclusion rule is what makes a *shared* hub safe on a healthcare codebase. It is enforced in `.agents/skills/secure/SKILL.md` and tracked in `compliance/org-controls-audit.md`.
