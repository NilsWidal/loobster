...
- **Token UX:** the controls panel now deep-links "Create a fine-grained PAT" with the exact two settings spelled out (Only select repositories → this repo; Contents: Read and write). GitHub has no API to create PATs, so those clicks are the irreducible manual step.
