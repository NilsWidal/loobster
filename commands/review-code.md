...
3. Analyze changes for: security, performance, style, consistency, missing edge cases, dependency impacts, and integration risks.
   - **Token discipline (see `${CLAUDE_PLUGIN_ROOT}/reference/token-discipline.md`):** for a wide diff, delegate per-area review to `Agent` subagents (e.g. one per dimension or per large file). Each subagent receives its slice of the diff plus what to check, and returns **only** a findings list (each: severity + `file:line` + one-line fix) — never the raw hunks. The main thread merges those findings into the single prioritized action list below without re-reading the hunks the subagents already saw.
