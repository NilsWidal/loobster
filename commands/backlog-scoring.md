...
How the goal-loop (`/reppit-goal`) and `/make-plan` score and re-score backlog items so the loop always works the highest-leverage task. Scores live in each Claude Code Task's `metadata` (the backlog source of truth).
