...
4. **Review & learn:** model-judge the cycle's result against the goal's success criteria → **met / partial / not-met**; append a one-line learning to `plans/loop/<slug>.md` (re-summarize the log so it stays small); **re-score** the backlog from what this cycle taught (never overwrite user-set factors). **Emit signals**: if this cycle surfaced something other loops/teammates should know (a friction, an opportunity, a fact), write it to the shared hub per `${CLAUDE_PLUGIN_ROOT}/commands/signals.md` (stamp `author`, dedup first, **no PHI**). Mark any signal you acted on this cycle `acted`/`archived`.
