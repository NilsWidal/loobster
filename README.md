...
![version](https://img.shields.io/badge/version-0.15.1-3fb950)
![tests](https://img.shields.io/badge/tests-111%20passing-3fb950)
...
| **Fleet dashboard** | One **GitHub Pages** board for the whole team, aggregating every loop across **all recent branches** — with Pause / Resume / Stop buttons **and an interactive per-loop task board** (move statuses, add tasks from your phone) that commit edits back (no server, git is the only data plane) |
...
- **Editable with zero backend.** Paste a fine-grained PAT into the page (stored only in that browser; the panel deep-links the token-creation page with the exact two settings) and the **Pause / Resume / Stop** buttons commit a `status:` edit to the marker *on its branch* via the GitHub API. Loops check `origin` for upstream marker edits at every cycle trigger and obey them as if set locally — so a teammate can stop a loop running on your machine from their phone, asynchronously, with the whole exchange audit-logged as commits.
- **A real task board, same trick.** Each loop card renders a **kanban of its actual backlog** (in progress / open / parked / done, sorted by RICE) from committed task files at `plans/loop/<goal>-tasks/` — one file per item, mirrored by the loop at every checkpoint. Moving a status or **adding a task from the board** commits the edit to the loop's branch, and the loop adopts it next cycle (`TaskCreate`/`TaskUpdate`, re-scored, filed into Linear when the backlog is Linear-backed). Filing work for an agent from your phone is now the same motion as stopping it.
